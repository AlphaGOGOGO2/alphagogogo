...
export const getBlogPostBySlug = async (slug: string): Promise<BlogPost | null> => {
  try {
    ...
    // 발행 시간에 대한 여유를 더 늘림 (5분)
    const now = new Date();
    now.setMinutes(now.getMinutes() + 5); // 5분 여유 추가로 변경 (시간 차이 문제 해결)
    const nowIsoString = now.toISOString();
    
    console.log(`[블로그] "${slug}" 슬러그 조회 기준 시간: ${nowIsoString}`);
    
    // 먼저 슬러그로 글이 존재하는지만 확인 (시간 제약 없이)
    ...
    console.log(`[블로그] 발행 시간: ${postData.published_at}, 현재 시간(+5분): ${nowIsoString}`);
    
    // 발행 시간 체크 - 빠른 디버깅을 위해 시간 형식을 쉽게 읽을 수 있게 변환
    ...
    // 현재 시간이 발행 시간보다 이후인지 확인
    if (publishedAt > nowDate) {
      const minutesDiff = Math.round((publishedAt.getTime() - nowDate.getTime()) / (1000 * 60));
      console.log(`[블로그] "${slug}" 슬러그 글은 아직 발행 예정 상태: ${publishedAt.toLocaleString('ko-KR')}, ${minutesDiff}분 남음`);
      toast.warning(`이 포스트는 ${publishedAt.toLocaleString('ko-KR')}에 발행될 예정입니다 (약 ${minutesDiff}분 후)`);
      return null;
    }
