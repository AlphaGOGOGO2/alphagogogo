...
export function Community() {
  ...
  return (
    <section id="community" className="py-24 px-6 md:px-8 bg-gradient-to-b from-[#512da8] to-[#4527a0] relative">
      {/* Section separator - top wavy line */}
      <div className="absolute top-0 left-0 right-0 h-12 overflow-hidden">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none" className="absolute bottom-0 w-full h-16 text-[#5e35b1] fill-current">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {communityFeatures.map((feature, index) => (
            <div 
              key={index}
              className={`p-6 rounded-xl border border-purple-100 transition-all hover:shadow-lg ${feature.color}`}
            >
        ...
        <div className="bg-white/10 backdrop-blur-sm p-8 md:p-12 rounded-2xl shadow-lg border border-white/20">
          <div className="md:flex items-center justify-between">
            ...
            <div className="flex flex-wrap gap-4">
              <Link 
                to="/community/join" 
                className="px-6 py-3 rounded-lg bg-purple-900 text-white font-medium hover:shadow-lg hover:shadow-purple-900/30 transition-all transform hover:-translate-y-0.5"
              >
                가입하기
              </Link>
              <Link 
